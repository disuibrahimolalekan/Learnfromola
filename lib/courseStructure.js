...
export async function getModules() {
  ...
  (chaptersResult.data || []).forEach((row) => {
    ...
    const numbers = chaptersByModule[row.module_number] || [];
    numbers.push(row.chapter_number);
    chaptersByModule[row.module_number] = numbers;
  });
  ...
  return (modulesResult.data || []).map((m) => ({
    ...
    chapterNumbers: (chaptersByModule[m.number] || []).sort((a, b) => a - b),
  }));
}
